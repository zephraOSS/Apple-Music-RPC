...
window.api.receive("new-update-available", (_e, data) => {
    newModal(
        (<LangString>langString).settings.modal["newUpdate"].title,
        (<LangString>langString).settings.modal[
            "newUpdate"
        ].description.replace("%version%", data.version),
        [
            {
                text: (<LangString>langString).settings.modal["newUpdate"]
                    .buttons.downandinst,
                style: "btn-grey",
                events: [
                    {
