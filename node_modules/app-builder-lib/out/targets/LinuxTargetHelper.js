...
class LinuxTargetHelper {
  ...
  async computeDesktopIcons() {
    ...
    const sources = [platformSpecificBuildOptions.icon, (_b = (_a = config.mac) === null || _a === void 0 ? void 0 : _a.icon) !== null && _b !== void 0 ? _b : config.icon].filter(str => !!str); // If no explicit sources are defined, fallback to buildResources directory, then default framework icon

    const fallbackSources = [(_c = config.directories) === null || _c === void 0 ? void 0 : _c.buildResources, ...(0, _builderUtil().asArray)(packager.getDefaultFrameworkIcon())].filter(async filepath => filepath && (await (0, _builderUtil().exists)(filepath))); // need to put here and not as default because need to resolve image size
